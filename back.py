...
def process_df(df):
    ...
    print(1)
    #print(new_df_ohlc.tail(1).index)
    ...
    peak_all = process_peak_bot(peak_x)
    bot_all = process_peak_bot(bot_x)
    ...
    return peak_all,bot_all,plot_all

import csv
import os
style.use('ggplot')
directory = 'stock_nyse/'
file_list=[]
with open('output2.csv', 'w+', newline='') as csvfile:
        ...
        writer.writerow(['ticker', 'peak_cycle_date','bot_cycle_date','peak_pattern','bot_pattern','peak_date','bot_date','peak_hit','bot_hit','all_peak_count','all_bot_count'])
    ...
    try:
        if flag:
            ...
            with open('output2.csv', 'a+', newline='') as csvfile:
                ...
                writer.writerow([file.replace(".csv",""),result[0][4],result[1][4],result[0][3],result[1][3],peak_date_array,bot_date_array,result[0][2],result[1][2],result[0][6],result[1][6]])
    except:
        continue
    #if file=="FWP.csv":
    #    flag=True
